...
export default {
  ...
  data() {
    return {
      ...
      menus: [
        ...
        { path: '/admin/recruitment-campaigns', label: '表单系统', icon: DocumentChecked, permissions: ['recruitment-campaign:list'] },
        { path: '/admin/form-submissions', label: '表单记录', icon: Tickets, permissions: ['recruitment-campaign:detail'] },
  ...
  computed: {
    visibleMenus() {
      return this.menus.filter((item) => item.path === '/' || hasAnyPermission(item.permissions || []))
    },
    pageTitle() {
