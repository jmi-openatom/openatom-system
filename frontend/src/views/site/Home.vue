<template>
  <div class="home-page" v-loading="loading">
      ...
      <div class="container hero__inner">
        <div class="hero__content">
          ...
          <div class="hero__actions">
            <el-button type="primary" size="large" :icon="DataAnalysis" @click="scrollTo('activities')">
              查看活动
            </el-button>
            <el-button size="large" :icon="UserFilled" @click="scrollTo('people')">了解成员</el-button>
          </div>
        </div>
        ...
        <div class="command-panel">
          ...
          <el-empty v-if="!metrics.length" description="暂无社团统计数据" :image-size="72" />
    ...
    <section class="container section club-brief">
      ...
      <div class="brief-carousel-wrapper">
        <el-carousel
            v-if="focusAreas.length"
            :interval="4000"
            type="card"
            height="300px"
            class="brief-carousel"
            arrow="never"
        >
          <el-carousel-item v-for="item in focusAreas" :key="item.title">
            <article class="brief-card reveal-card">
              <div class="icon-wrapper">
                <el-icon><component :is="focusIcon(item.icon)" /></el-icon>
              </div>
              <h3>{{ item.title }}</h3>
    ...
    <section class="activity-band" id="activities">
      <div class="container section">
        ...
        <div class="activity-timeline">
          <article v-for="activity in activities" :key="activity.title" class="activity-item reveal-card" @click="$router.push(`/activities/${activity.id}`)">
            <time>{{ activity.date }}</time>
            <div>
              <h3>{{ activity.title }}</h3>
              <p>{{ activity.description }}</p>
            </div>
            <el-tag :type="statusType(activity.status)" effect="plain">{{ statusText(activity.status) }}</el-tag>
          </article>
        </div>
    ...
    <section class="container section people-section" id="people">
      <div class="section-heading reveal-block">
        <span>成员信息</span>
        <h2>主要人员</h2>
    ...
    <section class="container section achievements-section" id="achievements">
      <div class="section-heading reveal-block">
        <span>成果展示</span>
        <h2>比赛获奖展示</h2>
      ...
      <div class="award-grid">
        <article v-for="award in awards" :key="award.id || `${award.year}-${award.title}`" class="award-card reveal-card">
          <div class="award-card__year">{{ award.year }}</div>
          <h3>{{ award.title }}</h3>
          <p>{{ award.competitionName }}</p>
          <strong>{{ award.awardLevel || award.teamName }}</strong>
        </article>
      </div>
