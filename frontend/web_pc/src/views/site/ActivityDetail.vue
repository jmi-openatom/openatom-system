            ...
            <aside class="signup-panel">
                ...
                <template v-else>
                    <el-form label-position="top">
                        <el-form-item v-for="field in fields" :key="field.label" :label="field.label"
                                      :required="field.required">
                            <el-select v-if="field.type === 'select'" v-model="form[field.label]" placeholder="请选择">
                                <el-option v-for="option in field.options || []" :key="option" :label="option"
                                           :value="option"/>
                            </el-select>
                            ...
                            <el-input v-else-if="field.type === 'textarea'" v-model="form[field.label]" :rows="3"
                                      type="textarea"/>
...
export default {
    ...
    computed: {
        ...
        fields() {
            try {
                const parsed = typeof this.activity.registrationFields === 'string'
