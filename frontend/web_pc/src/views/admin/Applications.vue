  ...
  <div class="admin-page">
    <div class="toolbar">
      <div class="toolbar__filters">
        <el-input v-model="query.keyword" clearable placeholder="搜索申请人/社团" style="width: 220px" @keyup.enter="fetchList" />
      ...
      <el-button type="success" :disabled="!selection.length" @click="batchApprove">批量通过</el-button>
    </div>
    <el-table v-loading="loading" :data="rows" class="admin-table" @selection-change="selection = $event">
      ...
      <el-table-column label="操作" width="260" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="canPreScreen(row)" link type="success" @click="openApproval(row, 'approve')">通过</el-button>
          <el-button v-if="canPreScreen(row)" link type="danger" @click="openApproval(row, 'reject')">驳回</el-button>
          <el-button v-if="canScheduleInterview(row)" link type="primary" @click="openInterview(row)">安排面试</el-button>
          <el-button v-if="canFinalDecision(row)" link type="warning" @click="openFinalDecision(row)">终审</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="approvalVisible" title="处理申请" width="520px">
      <el-form :model="approvalForm" label-width="86px">
        <el-form-item label="处理结果">
          <el-tag :type="approvalForm.action === 'approve' ? 'success' : 'danger'">{{ approvalLabel(approvalForm.action) }}</el-tag>
        </el-form-item>
      ...
      <div v-if="currentProfileRow" class="profile-panel">
        <div class="profile-header">
          <div>
            ...
            <p>{{ currentProfileRow.clubName || '-' }} / {{ currentProfileRow.preferredDepartment || '未选择部门' }}</p>
          </div>
          <el-tag :type="statusType(currentProfileRow.status)">{{ currentProfileRow.status || '-' }}</el-tag>
        </div>
        <el-descriptions :column="2" border>
          <el-descriptions-item label="申请ID">{{ currentProfileRow.id }}</el-descriptions-item>
          <el-descriptions-item label="提交时间">{{ formatDateTime(currentProfileRow.createdAt) }}</el-descriptions-item>
        </el-descriptions>
    ...
    <el-dialog v-model="interviewVisible" title="安排面试" width="560px">
      <el-form :model="interviewForm" label-width="98px">
        <el-form-item label="申请ID"><el-input v-model="interviewForm.applicationId" disabled /></el-form-item>
        <el-form-item label="开始时间"><el-date-picker v-model="interviewForm.scheduledStartAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" /></el-form-item>
        <el-form-item label="结束时间"><el-date-picker v-model="interviewForm.scheduledEndAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" /></el-form-item>
        <el-form-item label="地点/链接"><el-input v-model="interviewForm.location" /></el-form-item>
        <el-form-item label="方式"><el-select v-model="interviewForm.mode"><el-option label="线下" value="offline" /><el-option label="线上" value="online" /></el-select></el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="finalVisible" title="终审决策" width="600px">
      <el-form :model="finalForm" label-width="98px">
        <el-form-item label="申请ID"><el-input v-model="finalForm.applicationId" disabled /></el-form-item>
        ...
        <el-form-item v-if="finalForm.decision === 'approved'" label="部门">
          <el-select v-model="finalForm.departmentId" filterable placeholder="请选择部门" style="width: 320px">
            <el-option v-for="item in departments" :key="item.id" :label="item.name || `部门 ${item.id}`" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="finalForm.decision === 'approved'" label="岗位">
          <el-select v-model="finalForm.positionId" filterable placeholder="请选择岗位" style="width: 320px">
            <el-option v-for="item in positions" :key="item.id" :label="item.name || `岗位 ${item.id}`" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="备注"><el-input v-model="finalForm.comment" type="textarea" :rows="3" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="finalVisible = false">取消</el-button>
        <el-button type="primary" :loading="finalSubmitting" @click="submitFinalDecision">提交</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      interviewForm: { applicationId: '', round: 1, scheduledStartAt: '', scheduledEndAt: '', location: '', mode: 'offline' },
      finalVisible: false,
      finalSubmitting: false,
      finalForm: { applicationId: '', clubId: '', decision: 'approved', departmentId: undefined, positionId: undefined, comment: '' },
      departments: [],
      positions: []
    }
  },
  ...
  methods: {
    ...
    openInterview(row) {
      this.interviewForm = { applicationId: row.id, round: 1, scheduledStartAt: '', scheduledEndAt: '', location: '', mode: 'offline' }
    ...
    async openFinalDecision(row) {
      ...
      this.positions = []
      if (row.clubId) {
        const [departments, positions] = await Promise.all([clubApi.departments(row.clubId), clubApi.positions(row.clubId)])
    ...
    async submitFinalDecision() {
      ...
      try {
        await membershipApi.finalDecision(this.finalForm.applicationId, {
          ...
          departmentId: this.finalForm.decision === 'approved' ? this.finalForm.departmentId : undefined,
          positionId: this.finalForm.decision === 'approved' ? this.finalForm.positionId : undefined,
