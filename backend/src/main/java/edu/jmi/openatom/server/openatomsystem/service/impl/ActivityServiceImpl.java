...
public class ActivityServiceImpl implements ActivityService {
  ...
  public ApiResponse<List<ClubActivity>> list(String status) {
    ...
    return ApiResponse.success(clubActivityMapper.selectList(wrapper));
  }
  ...
  public ApiResponse<String> create(RequestCreateActivityDTO request) {
    ...
    if (status == null) {
      return ApiResponse.error(400, "活动状态不合法");
    }
  ...
  public ApiResponse<String> update(Integer activityId, RequestUpdateActivityDTO request) {
    ...
    if (request.getDescriptionMarkdown() != null) {
      activity.setDescriptionMarkdown(request.getDescriptionMarkdown());
    }
    if (request.getActivityAt() != null) {
      activity.setActivityAt(Times.parseTimestamp(request.getActivityAt()));
    }
    if (request.getEndAt() != null) {
      activity.setEndAt(Times.parseTimestamp(request.getEndAt()));
    }
    ...
    if (request.getRegistrationRequired() != null) {
      activity.setRegistrationRequired(request.getRegistrationRequired());
    }
    if (request.getRegistrationStartAt() != null) {
      activity.setRegistrationStartAt(Times.parseTimestamp(request.getRegistrationStartAt()));
    }
    if (request.getRegistrationEndAt() != null) {
      activity.setRegistrationEndAt(Times.parseTimestamp(request.getRegistrationEndAt()));
    }
    if (request.getRegistrationFields() != null) {
      activity.setRegistrationFields(Jsons.stringify(request.getRegistrationFields()));
    }
  ...
  public ApiResponse<String> register(Integer activityId, RequestActivityRegistrationDTO request) {
    if (!StpUtil.isLogin()) {
      return ApiResponse.error(401, "请先登录后再报名");
    }
    ClubActivity activity = findActivity(activityId);
    ...
    activityRegistrationMapper.insert(
        ActivityRegistration.builder()
            .activityId(activityId)
            .userId(userId)
            .formData(Jsons.stringify(request == null ? null : request.getFormData()))
            .status("submitted")
            .build());
  ...
  public ApiResponse<List<ActivityRegistration>> registrations(Integer activityId) {
    if (findActivity(activityId) == null) {
      return ApiResponse.error(404, "活动不存在");
    }
    return ApiResponse.success(
        activityRegistrationMapper.selectList(
            new LambdaQueryWrapper<ActivityRegistration>()
                .eq(ActivityRegistration::getActivityId, activityId)
  ...
  private ClubActivity findActivity(Integer activityId) {
    ...
    return clubActivityMapper.selectOne(
        new LambdaQueryWrapper<ClubActivity>()
