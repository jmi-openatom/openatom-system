...
public class RecruitmentCampaignServiceImpl implements RecruitmentCampaignService {
  ...
  public ApiResponse<String> create(Integer clubId, RequestCreateRecruitmentCampaignDTO request) {
    ...
    RecruitmentCampaign campaign =
        RecruitmentCampaign.builder()
            .clubId(clubId)
            .name(request.getName())
            .applyStartAt(Times.parseTimestamp(request.getApplyStartAt()))
            .applyEndAt(Times.parseTimestamp(request.getApplyEndAt()))
            .interviewStartAt(Times.parseTimestamp(request.getInterviewStartAt()))
            .interviewEndAt(Times.parseTimestamp(request.getInterviewEndAt()))
            .resultPublishAt(Times.parseTimestamp(request.getResultPublishAt()))
            .targetGrades(Jsons.stringify(request.getTargetGrades()))
  ...
  public ApiResponse<String> update(
    ...
    if (request.getApplyStartAt() != null) {
      campaign.setApplyStartAt(Times.parseTimestamp(request.getApplyStartAt()));
    }
    if (request.getApplyEndAt() != null) {
      campaign.setApplyEndAt(Times.parseTimestamp(request.getApplyEndAt()));
    }
    if (request.getInterviewStartAt() != null) {
      campaign.setInterviewStartAt(Times.parseTimestamp(request.getInterviewStartAt()));
    }
    if (request.getInterviewEndAt() != null) {
      campaign.setInterviewEndAt(Times.parseTimestamp(request.getInterviewEndAt()));
    }
    if (request.getResultPublishAt() != null) {
      campaign.setResultPublishAt(Times.parseTimestamp(request.getResultPublishAt()));
    }
    if (request.getTargetGrades() != null) {
      campaign.setTargetGrades(Jsons.stringify(request.getTargetGrades()));
    }
    if (request.getMaxApplicants() != null) {
      campaign.setMaxApplicants(request.getMaxApplicants());
    }
    if (request.getLoginRequired() != null) {
      campaign.setLoginRequired(request.getLoginRequired());
    }
    if (request.getFormSchema() != null) {
      campaign.setFormSchema(Jsons.stringify(request.getFormSchema()));
    }
  ...
  @Override
  public ApiResponse<String> publish(Integer campaignId) {
    return updateStatus(campaignId, "open", "招新计划发布成功");
  }

  @Override
  public ApiResponse<String> close(Integer campaignId) {
    return updateStatus(campaignId, "closed", "招新计划关闭成功");
  }
  ...
  private RecruitmentCampaign findCampaign(Integer campaignId) {
    return campaignId == null ? null : recruitmentCampaignMapper.selectById(campaignId);
  }
}
