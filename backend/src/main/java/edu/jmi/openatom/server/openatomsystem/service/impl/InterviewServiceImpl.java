...
public class InterviewServiceImpl implements InterviewService {
  ...
  public ApiResponse<List<Interview>> list(
    ...
    if (campaignId != null) {
      List<Integer> applicationIds =
          applicationMapper
              ...
              .stream()
              .map(MembershipApplication::getId)
              .toList();
    ...
    return ApiResponse.success(interviewMapper.selectList(wrapper));
  }
  ...
  public ApiResponse<String> create(RequestCreateInterviewDTO request) {
    ...
    if (row <= 0) {
      return ApiResponse.error("面试创建失败");
    }
  ...
  public ApiResponse<String> update(Integer interviewId, RequestUpdateInterviewDTO request) {
    Interview interview = findInterview(interviewId);
    if (interview == null) {
      return ApiResponse.error(404, "面试不存在");
    }
    if (request.getRound() != null) {
      interview.setRound(request.getRound());
    }
    if (request.getScheduledStartAt() != null) {
      interview.setScheduledStartAt(Times.parseTimestamp(request.getScheduledStartAt()));
    }
    if (request.getScheduledEndAt() != null) {
      interview.setScheduledEndAt(Times.parseTimestamp(request.getScheduledEndAt()));
    }
    ...
    if (request.getInterviewerIds() != null) {
      bindInterviewers(interviewId, request.getInterviewerIds());
    }
    return ApiResponse.success("面试更新成功");
  }

  @Override
  public ApiResponse<String> confirm(Integer interviewId) {
    return changeStatus(interviewId, "confirmed", "面试确认成功");
  }
  ...
  public ApiResponse<String> feedback(Integer interviewId, RequestInterviewFeedbackDTO request) {
    ...
    interviewFeedbackMapper.insert(
        InterviewFeedback.builder()
            .interviewId(interviewId)
            .interviewerId(StpUtil.getLoginIdAsInt())
            .scores(Jsons.stringify(request.getScores()))
  ...
  private void bindInterviewers(Integer interviewId, List<Integer> interviewerIds) {
    interviewInterviewerMapper.delete(
        new LambdaQueryWrapper<InterviewInterviewer>()
  ...
  private Interview findInterview(Integer interviewId) {
    return interviewId == null ? null : interviewMapper.selectById(interviewId);
  }
}
