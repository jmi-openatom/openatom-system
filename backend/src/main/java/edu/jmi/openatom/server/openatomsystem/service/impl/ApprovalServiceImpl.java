...
public class ApprovalServiceImpl implements ApprovalService {
  private static final List<String> ACTIONS =
      List.of("approve", "reject", "transfer", "request_more_info");
  ...
  public ApiResponse<List<ApprovalRecord>> records(Integer applicationId) {
    ...
    return ApiResponse.success(
        approvalRecordMapper.selectList(
  ...
  public ApiResponse<String> approve(Integer applicationId, RequestApprovalActionDTO request) {
    ...
    if (!ACTIONS.contains(request.getAction())) {
      return ApiResponse.error(400, "审批动作不合法");
    }
    approvalRecordMapper.insert(
        ApprovalRecord.builder()
            .applicationId(applicationId)
            .node(request.getNode())
            .action(request.getAction())
            .operatorId(StpUtil.getLoginIdAsInt())
  ...
  public ApiResponse<String> batchApprove(RequestBatchApprovalDTO request) {
    if (request.getApproval() == null) {
      return ApiResponse.error(400, "审批参数不能为空");
    }
  ...
  private String resolveNextStatus(String currentStatus, RequestApprovalActionDTO request) {
    if ("reject".equals(request.getAction())) {
      return "pre_screen".equals(request.getNode()) ? "pre_screen_rejected" : "rejected";
    }
    ...
    if ("pre_screen".equals(request.getNode())) {
      return "pre_screen_passed";
    }
    return currentStatus;
  }
}
