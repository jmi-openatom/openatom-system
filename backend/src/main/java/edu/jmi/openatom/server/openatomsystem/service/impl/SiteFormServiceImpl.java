...
public class SiteFormServiceImpl implements SiteFormService {
  ...
  public ApiResponse<List<SiteForm>> listByClub(Integer clubId) {
    ...
    return ApiResponse.success(siteFormMapper.selectByClubIdOrdered(clubId));
  }
  ...
  public ApiResponse<String> update(Integer formId, RequestUpdateSiteFormDTO request) {
    ...
    if (request.getStatus() != null) {
      if (!STATUSES.contains(request.getStatus())) return ApiResponse.error(400, "表单状态不合法");
  ...
  @Override
  public ApiResponse<String> publish(Integer formId) { return updateStatus(formId, "open", "表单发布成功"); }

  @Override
  public ApiResponse<String> close(Integer formId) { return updateStatus(formId, "closed", "表单关闭成功"); }
