...
public class SaTokenConfigure implements WebMvcConfigurer {
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		registry.addInterceptor(new SaInterceptor(handle -> {
			...
			StpUtil.checkLogin();
		})).addPathPatterns("/**").excludePathPatterns("/auth/register", "/auth/login", "/auth/miniapp-login", "/auth/refresh-token", "/auth/qq-bind", "/auth/qq-bind/confirm", "/auth/qq-bind/status", "/bot/leave-applications", "/bot/leave-applications/**", "/site/**", "/clubs/{clubId}/departments", "/files/avatars/**");
	}
