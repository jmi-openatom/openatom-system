...
public class ExitApplicationServiceImpl implements ExitApplicationService {
  ...
  public Result<List<ExitApplication>> list() {
    return Result.success(exitApplicationMapper.selectAllOrdered());
  }

  @Override
  public Result<String> create(RequestCreateExitApplicationDTO request) {
    ClubMembership membership = membershipMapper.selectById(request.getMembershipId());
    if (membership == null) return Result.error(404, "成员不存在");
    if ("left".equals(membership.getStatus())) return Result.error(422, "成员已退社");
    ...
    return Result.success("退社申请提交成功");
  }

  @Override
  public Result<ExitApplication> detail(Integer exitApplicationId) {
    ExitApplication application = findExitApplication(exitApplicationId);
    return application == null ? Result.error(404, "退社申请不存在") : Result.success(application);
  }

  @Override
  @Transactional(rollbackFor = Exception.class)
  public Result<String> approve(Integer exitApplicationId) {
    ExitApplication application = findExitApplication(exitApplicationId);
    if (application == null) return Result.error(404, "退社申请不存在");
    ...
    return Result.success("退社申请已通过");
  }

  @Override
  public Result<String> reject(Integer exitApplicationId, String comment) {
    ExitApplication application = findExitApplication(exitApplicationId);
    if (application == null) return Result.error(404, "退社申请不存在");
    application.setStatus("rejected");
    exitApplicationMapper.updateById(application);
    return Result.success("退社申请已驳回");
  }
