CREATE DATABASE IF NOT EXISTS `openatom-system`
...
(
    ...
    `type`   VARCHAR(20)  DEFAULT NULL COMMENT '类型: menu/page/button/api/data',
    `path`   VARCHAR(255) DEFAULT NULL COMMENT '路由地址/接口路径',
    `method` VARCHAR(10)  DEFAULT NULL COMMENT '请求方式',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_perm_code` (`code`),
    KEY `idx_perm_path_method` (`path`, `method`)
) ENGINE = InnoDB
...
(
    ...
    `code`               VARCHAR(50)  NOT NULL COMMENT '社团编码',
    `category`           VARCHAR(50)       DEFAULT NULL COMMENT '分类',
    `description`        TEXT              DEFAULT NULL COMMENT '描述',
    `logo_url`           VARCHAR(255)      DEFAULT NULL COMMENT 'Logo URL',
    `president_user_id`  INT               DEFAULT NULL COMMENT '负责人用户ID',
    `status`             VARCHAR(20)       DEFAULT 'active' COMMENT '社团状态: active/inactive',
    `recruitment_status` VARCHAR(20)       DEFAULT 'closed' COMMENT '招新状态: open/closed',
    `created_at`         TIMESTAMP         DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`         TIMESTAMP    NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_club_code` (`code`),
    KEY `idx_club_status` (`status`),
    KEY `idx_club_recruitment_status` (`recruitment_status`)
) ENGINE = InnoDB
...
(
    ...
    PRIMARY KEY (`id`),
    KEY `idx_department_club_id` (`club_id`)
) ENGINE = InnoDB
...
(
    ...
    `updated_at`    TIMESTAMP    DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_position_club_id` (`club_id`),
...
(
    ...
    KEY `idx_position_role_role_id` (`role_id`)
) ENGINE = InnoDB
...
(
    `id`                 INT          NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `club_id`            INT          NOT NULL COMMENT '社团ID',
    `name`               VARCHAR(100) NOT NULL COMMENT '招新计划名称',
    ...
    `updated_at`         TIMESTAMP    NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_campaign_club_id` (`club_id`),
...
(
    ...
    `club_id`                     INT       NOT NULL COMMENT '社团ID',
    `user_id`                     INT            DEFAULT NULL COMMENT '申请用户ID，匿名提交时可为空',
    `first_choice_department_id`  INT            DEFAULT NULL COMMENT '第一志愿部门ID',
    `second_choice_department_id` INT            DEFAULT NULL COMMENT '第二志愿部门ID',
    `status`                      VARCHAR(40)    DEFAULT 'draft' COMMENT '申请状态',
    `profile`                     JSON           DEFAULT NULL COMMENT '申请表单扩展信息',
    `created_at`                  TIMESTAMP      DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`                  TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_application_campaign_id` (`campaign_id`),
    KEY `idx_application_club_id` (`club_id`),
...
(
    ...
    `anonymous_contact` VARCHAR(100)      DEFAULT NULL COMMENT '匿名联系方式',
...
(
    ...
    `action`         VARCHAR(40)  NOT NULL COMMENT '审批动作',
    `operator_id`    INT          DEFAULT NULL COMMENT '操作人ID',
    `comment`        VARCHAR(500) DEFAULT NULL COMMENT '审批意见',
    `created_at`     TIMESTAMP    DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_approval_application_id` (`application_id`),
    KEY `idx_approval_operator_id` (`operator_id`)
) ENGINE = InnoDB
...
(
    ...
    `mode`               VARCHAR(20)  DEFAULT NULL COMMENT '方式: online/offline',
...
(
    ...
    KEY `idx_interviewer_user_id` (`interviewer_id`)
) ENGINE = InnoDB
...
(
    `id`             INT          NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `interview_id`   INT          NOT NULL COMMENT '面试ID',
    `interviewer_id` INT          DEFAULT NULL COMMENT '面试官用户ID',
    `scores`         JSON         DEFAULT NULL COMMENT '评分JSON',
    `suggestion`     VARCHAR(30)  DEFAULT NULL COMMENT '建议: pass/reject/waitlist',
...
(
    ...
    KEY `idx_membership_featured` (`featured`, `sort_order`)
) ENGINE = InnoDB
...
(
    ...
    `status`                VARCHAR(30)       DEFAULT 'draft' COMMENT '状态: draft/published/closed/cancelled',
    ...
    KEY `idx_activity_registration_required` (`registration_required`)
) ENGINE = InnoDB
...
(
    `id`            INT          NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `membership_id` INT          NOT NULL COMMENT '成员关系ID',
    `reason`        VARCHAR(255) NOT NULL COMMENT '退社原因',
    `description`   TEXT         DEFAULT NULL COMMENT '说明',
    `status`        VARCHAR(30)  DEFAULT 'submitted' COMMENT '状态: submitted/approved/rejected',
    `created_at`    TIMESTAMP    DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`    TIMESTAMP    DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_exit_membership_id` (`membership_id`),
...
(
    ...
    `receiver_user_id` INT       NOT NULL COMMENT '接收人用户ID',
    `read_flag`        TINYINT   DEFAULT 0 COMMENT '是否已读',
    `read_at`          TIMESTAMP NULL DEFAULT NULL COMMENT '阅读时间',
    PRIMARY KEY (`notification_id`, `receiver_user_id`),
...
(
    ...
    `target_id`   VARCHAR(80)  DEFAULT NULL COMMENT '目标ID',
    `content`     TEXT         DEFAULT NULL COMMENT '操作内容',
    `created_at`  TIMESTAMP    DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_operation_operator_id` (`operator_id`),
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '分配角色权限', 'role:permission:assign', 'api', '/roles/{roleId}/permissions', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'role:permission:assign');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新用户状态', 'user:status:update', 'api', '/users/{userId}/status', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'user:status:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '重置用户密码', 'user:password:reset', 'api', '/users/{userId}/reset-password', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'user:password:reset');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '分配用户角色', 'user:role:assign', 'api', '/users/{userId}/roles', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'user:role:assign');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '导入用户与下载模板', 'user:import', 'api', '/users/import/**', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'user:import');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取用户社团关系', 'user:membership:list', 'api', '/users/{userId}/memberships', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'user:membership:list');
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新社团状态', 'club:status:update', 'api', '/clubs/{clubId}/status', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'club:status:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新社团招新状态', 'club:recruitment-status:update', 'api', '/clubs/{clubId}/recruitment-status', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'club:recruitment-status:update');

-- 部门与岗位
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '查询部门列表', 'department:list', 'api', '/clubs/{clubId}/departments', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'department:list');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '创建部门', 'department:create', 'api', '/clubs/{clubId}/departments', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'department:create');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取部门详情', 'department:detail', 'api', '/departments/{departmentId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'department:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新部门', 'department:update', 'api', '/departments/{departmentId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'department:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '删除部门', 'department:delete', 'api', '/departments/{departmentId}', 'DELETE' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'department:delete');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '查询岗位列表', 'position:list', 'api', '/clubs/{clubId}/positions', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'position:list');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '创建岗位', 'position:create', 'api', '/clubs/{clubId}/positions', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'position:create');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取岗位详情', 'position:detail', 'api', '/positions/{positionId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'position:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新岗位', 'position:update', 'api', '/positions/{positionId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'position:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '删除岗位', 'position:delete', 'api', '/positions/{positionId}', 'DELETE' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'position:delete');

-- 招新管理
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '查询招新计划', 'recruitment-campaign:list', 'api', '/clubs/{clubId}/recruitment-campaigns', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'recruitment-campaign:list');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '发布招新计划', 'recruitment-campaign:publish', 'api', '/recruitment-campaigns/{campaignId}/publish', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'recruitment-campaign:publish');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '关闭招新计划', 'recruitment-campaign:close', 'api', '/recruitment-campaigns/{campaignId}/close', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'recruitment-campaign:close');
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取入会申请详情', 'application:detail', 'api', '/applications/{applicationId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新入会申请', 'application:update', 'api', '/applications/{applicationId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '提交草稿申请', 'application:submit', 'api', '/applications/{applicationId}/submit', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:submit');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '撤回入会申请', 'application:withdraw', 'api', '/applications/{applicationId}/withdraw', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:withdraw');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '查询审批记录', 'approval-record:list', 'api', '/applications/{applicationId}/approval-records', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'approval-record:list');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '审批入会申请', 'application:approve', 'api', '/applications/{applicationId}/approvals', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:approve');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '批量审批入会申请', 'application:batch-approve', 'api', '/applications/batch-approvals', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:batch-approve');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '终审决策', 'application:final-decision', 'api', '/applications/{applicationId}/final-decisions', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'application:final-decision');
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取面试详情', 'interview:detail', 'api', '/interviews/{interviewId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'interview:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新面试安排', 'interview:update', 'api', '/interviews/{interviewId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'interview:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '确认面试', 'interview:confirm', 'api', '/interviews/{interviewId}/confirm', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'interview:confirm');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '提交面试反馈', 'interview:feedback', 'api', '/interviews/{interviewId}/feedback', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'interview:feedback');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '完成面试', 'interview:complete', 'api', '/interviews/{interviewId}/complete', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'interview:complete');
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取活动详情', 'activity:detail', 'api', '/activities/{activityId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'activity:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新活动', 'activity:update', 'api', '/activities/{activityId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'activity:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '删除活动', 'activity:delete', 'api', '/activities/{activityId}', 'DELETE' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'activity:delete');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '查询活动报名', 'activity-registration:list', 'api', '/activities/{activityId}/registrations', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'activity-registration:list');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取成员详情', 'membership:detail', 'api', '/memberships/{membershipId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'membership:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新成员', 'membership:update', 'api', '/memberships/{membershipId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'membership:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '分配成员岗位', 'membership:position:assign', 'api', '/memberships/{membershipId}/assign-position', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'membership:position:assign');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '修改成员状态', 'membership:status:change', 'api', '/memberships/{membershipId}/change-status', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'membership:status:change');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '查询退社申请', 'exit-application:list', 'api', '/exit-applications', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'exit-application:list');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '提交退社申请', 'exit-application:create', 'api', '/exit-applications', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'exit-application:create');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '获取退社申请详情', 'exit-application:detail', 'api', '/exit-applications/{exitApplicationId}', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'exit-application:detail');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '通过退社申请', 'exit-application:approve', 'api', '/exit-applications/{exitApplicationId}/approve', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'exit-application:approve');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '驳回退社申请', 'exit-application:reject', 'api', '/exit-applications/{exitApplicationId}/reject', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'exit-application:reject');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '强制退社', 'membership:force-exit', 'api', '/memberships/{membershipId}/force-exit', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'membership:force-exit');
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '标记通知已读', 'notification:read', 'api', '/notifications/{notificationId}/read', 'POST' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'notification:read');
...
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '更新文书', 'document:update', 'api', '/office-documents/{documentId}', 'PATCH' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'document:update');
INSERT INTO `sys_permission` (`name`, `code`, `type`, `path`, `method`)
SELECT '导出文书', 'document:export', 'api', '/office-documents/{documentId}/export', 'GET' WHERE NOT EXISTS (SELECT 1 FROM `sys_permission` WHERE `code` = 'document:export');
...
SELECT '系统管理员', 'super_admin', 'all', '拥有系统全部接口权限'
WHERE NOT EXISTS (SELECT 1 FROM `sys_role` WHERE `code` = 'super_admin');

INSERT INTO `sys_role` (`name`, `code`, `data_scope`, `description`)
SELECT '正式成员', 'formal_member', 'self', '社团正式成员基础角色'
WHERE NOT EXISTS (SELECT 1 FROM `sys_role` WHERE `code` = 'formal_member');

INSERT INTO `sys_role` (`name`, `code`, `data_scope`, `description`)
SELECT '非正式成员', 'probationary_member', 'self', '社团非正式成员基础角色'
WHERE NOT EXISTS (SELECT 1 FROM `sys_role` WHERE `code` = 'probationary_member');
