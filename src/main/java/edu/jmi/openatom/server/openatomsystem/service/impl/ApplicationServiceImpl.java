...
public class ApplicationServiceImpl implements ApplicationService {
    ...
    if (keyword != null && !keyword.isBlank()) {
      ...
      if (userIds.isEmpty()) {
        return ApiResponse.success(
            PageDataDTO.<ResponseApplicationDTO>builder()
                .list(List.of())
                .page(current)
                .pageSize(size)
                .total(0L)
      ...
      wrapper.in(MembershipApplication::getUserId, userIds);
    }
    Page<MembershipApplication> applicationPage =
        applicationMapper.selectPage(new Page<>(current, size), wrapper);
    return ApiResponse.success(
        PageDataDTO.<ResponseApplicationDTO>builder()
            .list(toResponseList(applicationPage.getRecords()))
            .page(applicationPage.getCurrent())
            .pageSize(applicationPage.getSize())
            .total(applicationPage.getTotal())
            .build());
  }
  ...
  public ApiResponse<Integer> create(RequestCreateApplicationDTO request) {
      ...
      Long activeCount =
          applicationMapper.selectCount(
              new LambdaQueryWrapper<MembershipApplication>()
                  .eq(MembershipApplication::getCampaignId, request.getCampaignId())
                  .eq(MembershipApplication::getUserId, userId)
                  .ne(MembershipApplication::getStatus, "cancelled"));
      if (activeCount > 0) {
        return ApiResponse.error(409, "同一招新批次只能提交一次有效申请");
      }
    }
    MembershipApplication application =
        MembershipApplication.builder()
            ...
            .profile(Jsons.stringify(request.getProfile()))
            .status("submitted")
            .build();
    int row = applicationMapper.insert(application);
    return row > 0
        ? ApiResponse.success(application.getId(), "入会申请提交成功")
        : ApiResponse.error("入会申请提交失败");
  }
  ...
  public ApiResponse<String> update(Integer applicationId, RequestUpdateApplicationDTO request) {
    ...
    if (request.getProfile() != null) {
      application.setProfile(Jsons.stringify(request.getProfile()));
    }
  ...
  private ApiResponse<String> changeStatus(
      Integer applicationId, String requiredStatus, String targetStatus, String message) {
    MembershipApplication application = findApplication(applicationId);
    if (application == null) {
      return ApiResponse.error(404, "申请不存在");
    }
    if (!requiredStatus.equals(application.getStatus())) {
      return ApiResponse.error(422, "当前申请状态不允许执行该操作");
    }
  ...
  private ApiResponse<String> validateApplicationBase(
      RequestCreateApplicationDTO request, Integer userId) {
    if (request == null) {
      return ApiResponse.error(400, "申请信息不能为空");
    }
    ...
    if (!List.of("open", "published").contains(campaign.getStatus())) {
      return ApiResponse.error(400, "该招新计划未开放申请");
    }
    ...
    if (!Boolean.TRUE.equals(campaign.getLoginRequired())) {
      if (isBlank(readProfileValue(profile, "applicantName"))) {
        return ApiResponse.error(400, "请填写联系人姓名");
      }
      if (isBlank(readProfileValue(profile, "contact"))) {
        return ApiResponse.error(400, "请填写联系方式");
      }
    }
    ...
    if (!request.getClubId().equals(campaign.getClubId())) {
      return ApiResponse.error(400, "招新计划不属于当前社团");
    }
    if (clubMapper.selectById(request.getClubId()) == null) {
      return ApiResponse.error(404, "社团不存在");
    }
    if (request.getFirstChoiceDepartmentId() != null
        && departmentMapper.selectById(request.getFirstChoiceDepartmentId()) == null) {
      return ApiResponse.error(400, "第一志愿部门不存在");
    }
    if (request.getSecondChoiceDepartmentId() != null
        && departmentMapper.selectById(request.getSecondChoiceDepartmentId()) == null) {
  ...
  private ResponseApplicationDTO toResponse(
    ...
    return ResponseApplicationDTO.builder()
        ...
        .applicantName(
            user == null
                ? fallbackProfileValue(profile, "applicantName", "name", "realName")
        ...
        .firstChoiceDepartmentId(application.getFirstChoiceDepartmentId())
  ...
  private String fallbackProfileValue(Map<String, Object> profile, String... keys) {
    for (String key : keys) {
      String value = readProfileValue(profile, key);
      if (!isBlank(value)) {
        return value;
      }
    }
