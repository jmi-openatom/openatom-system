...
图片附件会先转成可在 Web 后台直接预览的 `data:image/...;base64,...` 内容，转换失败不会提交申请。提交成功时机器人会把私聊会话上下文提交给后端；后台通过或驳回后，后端会立即回调插件，机器人会通过私聊告知结果，驳回时会带上驳回理由。
