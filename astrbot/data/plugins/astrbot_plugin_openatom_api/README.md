...
项目的 `astrbot/data/cmd_config.json` 已关闭 AstrBot 全局 `t2i`，插件回复、普通模型回复和内置回复都保持文字输出。
...
用户必须先绑定 QQ。绑定后在群里艾特机器人并发送：

```text
我要请假
```

机器人会在群里提示用户去私聊，并在私聊里依次询问：
...
图片附件会先转成可在 Web 后台直接预览的 `data:image/...;base64,...` 内容，转换失败不会提交申请。提交成功时机器人会把私聊上下文提交给后端；后台通过或驳回后，后端会立即回调插件，机器人会在私聊里告知结果，驳回时会带上驳回理由。
